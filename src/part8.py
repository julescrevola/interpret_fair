...
def xper_method(model, eval_metric, dataset: pd.DataFrame, target_col: pd.Series, X_train: pd.DataFrame, y_train: pd.Series, X_test: pd.DataFrame, y_test: pd.Series):
    ...
    labels = list(dataset.drop(target_col, axis=1).columns)
...
def perm_imp(scoring: list, model, X_train: pd.Series, y_train: pd.Series):
    r_multi = permutation_importance(
        model, X_train, y_train, n_repeats=30, random_state=0, scoring=scoring
        )

    for metric in r_multi:
        print(f"Metric chosen: {metric}")
        r = r_multi[metric]
